...
# Zones de reporting (taxonomie client : EU + CAN / USA / APAC). Étendre ici.
COUNTRY_TO_ZONE = {
    # EU + CAN
...
def normalize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 4b. Campagnes : le type normalisé (PLA, PMax, Search Brand, Brex, Gen) est
    #     déduit du nom ; le pays est déduit du code (et sert au tableau région).
    #     Le libellé de campagne ne contient QUE le type (pas le pays).
    raw_campaign = df["campaign_type"].astype(str)
    parsed = raw_campaign.apply(parse_campaign)  # (type, pays)
    df["campaign_type"] = [
    ...
    # 5. Région + zone de reporting déduites du pays
    df["region"] = _derive_region(df["country"])
    df["zone"] = _derive_zone(df["country"])
...
# Tokens de type de campagne → libellé normalisé (token = mot exact).
CAMPAIGN_TYPE_TOKENS = {
...
def parse_campaign(raw):
    """Depuis un nom de campagne structuré (tokens séparés par « _ »), renvoie
    (type_normalisé|None, pays|None).

    Le type est le 1ᵉʳ token reconnu (PMax, PLA, Brex, Search Brand, Gen) ; le
    pays, le 1ᵉʳ code pays reconnu (FRA, USA, CAN, MULTI, PAN-ASIA…).
    """
    toks = [t for t in re.split(r"[_\s|]+", str(raw)) if t]
    type_label = next(
        (CAMPAIGN_TYPE_TOKENS[t.upper()] for t in toks
         if t.upper() in CAMPAIGN_TYPE_TOKENS), None)
    country = next(
        (COUNTRY_CODE_TO_COUNTRY[t.upper()] for t in toks
         if t.upper() in COUNTRY_CODE_TO_COUNTRY), None)
...
def _clean_numeric(series: pd.Series) -> pd.Series:
    """Convertit une colonne en nombres en gérant le format français :
    espaces (y compris insécables) comme séparateurs de milliers,
    virgule décimale, symbole € et autres parasites."""
    s = series.astype(str)

    # Suppression des espaces (normal, insécable, fine insécable) et du symbole €.
    ...
    # Virgule décimale → point. (On suppose les milliers déjà retirés.)
    s = s.str.replace(",", ".", regex=False)

    # Tout caractère restant non numérique (hors point et signe) est supprimé.
...
def _derive_zone(country: pd.Series) -> pd.Series:
    """Déduit la zone de reporting (EU + CAN / USA / APAC) depuis le pays."""
    norm_map = {_canon_key(k): v for k, v in COUNTRY_TO_ZONE.items()}
