...
def _inject_css() -> None:
    ...
    st.markdown(
        ...
        /* En-tête de page (titre + bouton période façon Looker) */
        .page-title {{font-size:24px; font-weight:800; color:{T['text']};
            line-height:1.15;}}
        .page-sub {{font-size:13px; color:{T['muted']}; font-weight:600;
            margin-top:3px;}}
        .title-rule {{border-bottom:3px solid {T['primary']};
            margin:10px 0 20px;}}
        /* Bouton de période (popover) : sobre, bordé, aligné à droite */
        div[data-testid="stPopover"] {{display:flex; justify-content:flex-end;}}
        div[data-testid="stPopover"] button {{
            background:#FFFFFF !important; color:{T['text']} !important;
            font-weight:700; border:1px solid #D7E2EE !important;
...
def sidebar_data_source() -> pd.DataFrame | None:
    ...
    if uploaded and (reload_clicked or sig != st.session_state.get("loaded_sig")):
        ...
        if frames:
            ...
            st.session_state["client_name"] = frames[0].attrs.get("client",
                                                                  "Générique")
    ...
    if df is not None and not df.empty:
        client = st.session_state.get("client_name", "Générique")
        st.sidebar.success(
            f"✅ {len(df):,} lignes chargées · "
            f"{df['date'].min():%d/%m/%Y} → {df['date'].max():%d/%m/%Y}"
            .replace(",", " "))
        st.sidebar.caption(f"🏷️ Client détecté : **{client}**")
        if st.sidebar.button("🗑️ Réinitialiser", width="stretch"):
            for k in ("data", "loaded_sig", "client_name"):
                st.session_state.pop(k, None)
            st.rerun()
    return df


@st.cache_data(show_spinner="Récupération des données Google Ads…")
def _gads_fetch_cached(customer_id: str, start: str, end: str) -> pd.DataFrame:
    """Appel API mémorisé par (compte, période) pour éviter les requêtes
    répétées (les identifiants, stables, viennent des Secrets)."""
    from core import google_ads
    return google_ads.fetch_raw(customer_id, start, end)


def _google_ads_section() -> None:
    """Section sidebar : import direct depuis l'API Google Ads (alternative aux
    fichiers). Les données passent par le même normalize() que les imports."""
    from core import google_ads

    st.sidebar.header("🔗 Google Ads")
    if not google_ads.configured():
        st.sidebar.caption(
            "Connecte ton compte : ajoute le bloc **[google_ads]** dans les "
            "**Secrets Streamlit** (developer_token, client_id, client_secret, "
            "refresh_token). Voir **GOOGLE_ADS_SETUP.md**.")
        return

    cid = st.sidebar.text_input("ID client (10 chiffres)", key="gads_cid",
                                placeholder="123-456-7890")
    c1, c2 = st.sidebar.columns(2)
    today = dt.date.today()
    start = c1.date_input("Du", value=today - dt.timedelta(days=27),
                          key="gads_start")
    end = c2.date_input("Au", value=today, key="gads_end")

    if st.sidebar.button("⬇️ Importer depuis Google Ads", width="stretch",
                         disabled=not cid):
        try:
            raw = _gads_fetch_cached(cid, str(start), str(end))
            if raw.empty:
                st.sidebar.warning("Aucune donnée sur cette période / ce compte.")
            else:
                norm = normalize(raw)
                st.session_state["data"] = norm
                st.session_state["loaded_sig"] = ("gads", cid, str(start),
                                                  str(end))
                st.session_state["client_name"] = norm.attrs.get(
                    "client", "Générique")
                st.sidebar.success(
                    f"✅ {len(norm):,} lignes importées de Google Ads."
                    .replace(",", " "))
                st.rerun()
        except Exception as exc:
            st.sidebar.error(f"Échec Google Ads : {exc}")

    with st.sidebar.expander("Trouver mon ID client"):
        if st.button("Lister mes comptes accessibles", key="gads_list"):
            ids = google_ads.accessible_customers()
            if ids:
                st.write("Comptes accessibles :")
                for i in ids:
                    st.code(f"{i[:3]}-{i[3:6]}-{i[6:]}")
            else:
                st.caption("Aucun compte listé (vérifie le developer token / "
                           "le login_customer_id).")
...
def page_header(df: pd.DataFrame, title: str | None = None):
    ...
    if start > end:
        ...
        return None
    return start, end, st.session_state.get("flt_comparison", "Aucune")
