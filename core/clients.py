...
# Regroupement par FAMILLE de levier (même type, nomenclature différente selon
# le client). Le libellé reste affiché tel quel ; la famille sert aux totaux /
# vues consolidées. Métier : DSA ≡ Brand Other ; Pure Brand ≡ Brex.
FAMILIES = {
    "DSA": "Brand Other", "Brand Other": "Brand Other",
    "Pure Brand": "Brex", "Brex": "Brex",
}


def family_of(label: str) -> str:
    """Famille d'un libellé de type (sinon le libellé lui-même)."""
    return FAMILIES.get(str(label), str(label))
