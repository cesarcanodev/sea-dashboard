...
# Types de campagne à reconnaître (token = mot, ponctuation ignorée).
...
def parse_campaign(raw, type_tokens=None):
    """Depuis un nom de campagne, renvoie (type_normalisé|None, pays|None).

    Gère plusieurs nomenclatures :
      * underscores   : KENZOCOUTURE_FLG_USA_PMAX_…  (type=PMax, pays=USA)
      * tirets/deux-points : « PMax: IM - IT/IT - TOP » (type=PMax, pays=IT)
                             « DSA - UK » · « IM - SK/EN - Search - Brand »
    La ponctuation est ignorée pour le type ; le pays est cherché dans chaque
    sous-jeton (un token « SK/EN » donne le pays « SK »).

    ``type_tokens`` : dictionnaire {TOKEN_MAJ: "Type"} propre au client détecté.
    """
    if type_tokens is None:
        type_tokens = CAMPAIGN_TYPE_TOKENS
